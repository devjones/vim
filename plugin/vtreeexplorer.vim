...
" Declare the title for the file explorer 
let Vtree_title = "TreeExplorer"
...
command! -nargs=0 -bar VTreeClose call s:VTree_Window_Close()
...
function! s:VTree_Window_Close()
    " Make sure the taglist window exists
    let winnum = bufwinnr(g:Vtree_title)
    if winnum == -1
        return
    endif
...
function! s:VTree_Window_Toggle()
    call s:Vtree_Log_Msg('Tlist_Window_Toggle()')
    " If taglist window is open then close it.
    let winnum = bufwinnr(g:Vtree_title)
    if winnum != -1
        call s:VTree_Window_Close()
        return
    endif
